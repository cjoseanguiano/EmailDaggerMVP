...
public class NetworkError extends Throwable {
    ...
    public boolean isAuthFailure() {
        return error instanceof retrofit2.adapter.rxjava.HttpException &&
                ((retrofit2.adapter.rxjava.HttpException) error).code() == HTTP_UNAUTHORIZED;
    }

    public boolean isResponseNull() {
        return error instanceof retrofit2.adapter.rxjava.HttpException && ((retrofit2.adapter.rxjava.HttpException) error).response() == null;
    }

    public String getAppErrorMessage() {
        if (this.error instanceof IOException) return NETWORK_ERROR_MESSAGE;
        if (!(this.error instanceof retrofit2.adapter.rxjava.HttpException)) return DEFAULT_ERROR_MESSAGE;
    ...
    protected String getJsonStringFromResponse(final retrofit2.Response<?> response) {
        try {
            ...
            ResponseStatus errorResponse = new Gson().fromJson(jsonString, ResponseStatus.class);
